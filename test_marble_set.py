...
class MarbleSetTestCase(unittest.TestCase):
	...
	def test_11_marbles_equal_to_1(self):
		marbles_equal_one = [x for x in self.marble_set.marbles if x == 1]
	...
	def test_choice_is_called(self):
		...
		odd_marble_options = [2, 4]
		marble_set = MarbleSet(marble_options=odd_marble_options, choice=fake_choice.choice)
		marbles_not_one = [x for x in self.marble_set.marbles if x in odd_marble_options]
